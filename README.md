...
- **YAML / JSON / TypeScript / Mermaid / Graphviz** — round-trip import and export for all formats, plus Mermaid and DOT diagram export
...
symflow mermaid workflow.json -o diagram.mmd

# Export as Graphviz DOT
symflow dot workflow.yaml
symflow dot workflow.json -o graph.dot

# Pipe to Graphviz
symflow dot workflow.yaml | dot -Tpng -o graph.png
```

Supported file formats: `.yaml`, `.yml`, `.json`, `.js`, `.ts`.

JS/TS files should export `definition` (and optionally `meta`), or use the `{name}Definition` / `{name}Meta` naming convention generated by `exportWorkflowTs`.
...
| `symflow/graphviz`   | Graphviz DOT digraph export                          | none                   | 0.7 kB      |
...
By default, each transition consumes and produces 1 token. With weighted arcs, transitions can consume or produce multiple tokens per firing:
...
engine.apply("manufacture"); // raw: 6-3=3, components: 0+2=2
engine.apply("manufacture"); // raw: 3-3=0, components: 2+2=4
engine.apply("assemble");    // components: 4-2=2, assembled: 0+1=1
```

Both `consumeWeight` and `produceWeight` default to `1` when omitted. Weights must be positive integers and are validated by `validateDefinition()`.

## Middleware

Middleware wraps the entire `apply()` lifecycle, letting you add cross-cutting concerns like logging, metrics, transactions, or access control:
...
const engine = new WorkflowEngine(definition, {
    middleware: [
        (ctx, next) => {
            ...
            const result = next();
            console.log(`Done, new marking:`, result);
            return result;
        },
    ],
});
...
engine.use((ctx, next) => {
    ...
    const result = next();
    console.log(`${ctx.transition.name} took ${performance.now() - start}ms`);
    return result;
});
```

Middleware executes in registration order (first registered = outermost wrapper). Each middleware must call `next()` to proceed with the transition, or return a marking directly to skip it.

The subject-driven `Workflow<T>` also supports middleware with access to the subject:
...
const workflow = createWorkflow<Order>(definition, {
    ...
    middleware: [
        (ctx, next) => {
            console.log(`Transitioning order ${ctx.subject.id}`);
            return next();
        },
    ],
});
...
Note: `can()` is **not** wrapped by middleware — guards are authorization, middleware is lifecycle.
...
Generates a `digraph` you can render with Graphviz, `dot`, or any DOT-compatible tool:
...
AND-split and AND-join transitions are rendered with an intermediate rectangle node so the fork/join structure is visible in the graph.
